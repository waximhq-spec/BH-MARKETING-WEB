...
export default function Navbar() {
  ...
  const navBorder = useTransform(
    scrollY,
    [0, 50],
    ["1px solid rgba(255, 255, 255, 0.05)", "1px solid rgba(0, 96, 57, 0.15)"]
  );
      ...
      <nav className="fixed top-0 left-0 right-0 z-[100] w-full px-4 sm:px-6 lg:px-10 pt-4 pb-2 transition-transform duration-500">
        <motion.div
          ...
          style={{
            ...
            border: navBorder,
            borderRadius: "16px",
            boxShadow: isScrolled ? "0 10px 30px rgba(0,0,0,0.4), inset 0 1px 0 rgba(255,255,255,0.06), inset 0 -10px 30px rgba(0, 96, 57,0.03)" : "none",
          }}
        >
          {/* Logo */}
          <Link
              ...
              <li key={link.name} className="relative group">
                ...
                >
                  <span className="relative">
                    ...
                    <span 
                      className="absolute -bottom-1 left-0 w-full h-[1px] bg-[#006039] origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-500 ease-[0.22,1,0.36,1]"
                    />
                  </span>
                </Link>
              </li>
        ...
        <div 
          className="absolute right-0 top-1/2 -translate-y-1/2 w-32 h-32 bg-[#006039]/25 blur-[45px] rounded-full pointer-events-none z-[-1]"
          style={{
            ...
            <ul className="flex flex-col items-center justify-center gap-10 text-5xl sm:text-6xl tracking-tight uppercase font-black text-center w-full px-6">
              {navLinks.map((link, i) => (
                ...
                >
                  <Link
                    href={link.href}
                    className="text-white hover:text-[#006039] transition-colors duration-200 drop-shadow-md cursor-none"
                    onClick={() => setMenuOpen(false)}
                  >
                    {link.name}
                  </Link>
