...
class Particle {
  ...
  constructor(width: number, height: number) {
    ...
    this.color = "#ffffff";
    this.glowColor = "rgba(154, 14, 31, 0.8)";
  }
  ...
  useEffect(() => {
      ...
      for (let i = 0; i < particles.length; i++) {
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distSq < minDistSq) {
            ...
            ctx.strokeStyle = `rgba(154, 14, 31, ${opacity})`;
            ctx.lineWidth = 0.8;
    ...
    const animate = () => {
      ...
      if (mouseRef.current.x > 0) {
        ...
        gradient.addColorStop(0, 'rgba(154, 14, 31, 0.12)');
        gradient.addColorStop(1, 'rgba(154, 14, 31, 0)');
